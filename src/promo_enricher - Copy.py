...
def _normalize_official_tag(text: str) -> Optional[str]:
    ...
    # Eliminamos acentos para checar
    t = text.strip().lower().replace("á","a").replace("é","e").replace("í","i").replace("ó","o").replace("ú","u")
    
    if "relampago" in t: return "⚡ Oferta Relámpago"
    if "imperdible" in t: return "💎 Imperdible"
    if "oferta del dia" in t: return "⏰ Oferta del Día"
    if "mas vendido" in t: return "🔥 Más Vendido"
    if "recomendado" in t: return "⭐ Recomendado"
...
def enrich_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    is_full = "FULL" in fetcher_tag.upper()
    ...
    if enriched["rating"] == 0.0 and enriched["reviews_count"] == 0:
        # Usamos t_check sin acentos para validar
        t_check = final_tag.lower().replace("á","a").replace("é","e")
