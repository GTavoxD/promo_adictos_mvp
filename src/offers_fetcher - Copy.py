...
def _normalize_text(text: str) -> str:
    """Elimina acentos para comparación segura."""
    ...
    for card in cards:
        try:
            ...
            if card.select_one(".poly-component__shipped-from-fulfillment, .andes-icon--fulfillment, span.ui-search-item__fulfillment-label, .poly-component__shipping-badge"):
                is_full = True
            # 2. Por texto explícito en toda la tarjeta
            elif "FULL" in _normalize_text(card.get_text()): 
                is_full = True
            # 3. Por atributos en svg
            elif card.select_one("svg[aria-label='Full'], svg[aria-label='full']"):
                is_full = True
